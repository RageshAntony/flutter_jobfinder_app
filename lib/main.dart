...
import 'package:flutter_jobfinder_app/search_page.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (context, constraints) {
      return OrientationBuilder(builder: (context, orientation) {
        ...
        //    create: (context) => GithubJobBloc(jobsRepo: JobsServices()),
            providers: [
              BlocProvider<JobPostBloc>(
                create: (BuildContext context) => JobPostBloc(jobsRepo: JobsServices()),
              ),
            ],
...
class _SplashScreenState extends State<SplashScreen> {
  ...
  Widget _list(List<JobPostByCompany$QueryRoot$JobPost> jobs) {
    return Expanded(
      child: ListView.builder(
        itemCount: jobs.length,
        scrollDirection: Axis.horizontal,
        itemBuilder: (_, index) {
          JobPostByCompany$QueryRoot$JobPost job = jobs[index];
          return  Row(
            children: [
              SizedBox(
              ...
              GestureDetector(
                ...
                child: _jobs(job.jobType.jobType),
              ),
            ],
          );
  ...
  Widget blocTest() {
    ...
    return BlocBuilder<JobPostBloc,JobPostState > (
      builder: (BuildContext context, JobPostState state) {
        print ('block api');
        ...
        if (state is JobPostError) {
          ...
          List<JobPostByCompany$QueryRoot$JobPost> jobs = state.jobs;
          print("job result: ${jobs[0].jobType.jobType}");
          return _list(jobs);
        }
  ...
  _loadAlbums() async {
    context.bloc<JobPostBloc>().add(JobPostEvent.fetchJobs);
  }
  @override
  Widget build(BuildContext context) {
...
/*
 BlocBuilder<GithubJobBloc, GithubJobEvent>(
        builder: (BuildContext context, GithubJobState state) {
          if (state is GithubJobListError) {
            final error = state.error;
            String message = '${error.message}\nTap to Retry.';
            return Text (message);
          }
          if (state is GithubJobLoaded) {
            List<Album> albums = state.githubjob;
            return _list(albums);
          }
          return Loading();
        })
